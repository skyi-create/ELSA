...
// Map internal types to Navbar subItems
const typeMap: Record<string, string> = {
    'NOTICE': '공지사항',
    'INTERVIEW': '회원사 소식',
    'ARCHIVE': '자료실',
    'EVENT': '공지사항', // Defaulting Event to Notice or separate if exists
    'Q&A': 'Q&A'
};
...
export const Community: React.FC<CommunityProps> = ({ onNavigate }) => {
  ...
  const handleItemClick = (type: string) => {
      if (onNavigate) {
          const target = typeMap[type] || '공지사항';
          onNavigate(target);
      }
  };
    ...
    <section id="community" className="py-24 bg-gray-50">
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12">
           ...
           <button 
             onClick={() => onNavigate && onNavigate('공지사항')}
             className="hidden md:flex items-center gap-2 text-[#2F4F4F] font-semibold hover:text-[#A9D95D] transition-colors"
           >
              전체 게시글 보기
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {communityPosts.map((post) => (
            <button 
              key={post.id} 
              onClick={() => handleItemClick(post.type)}
              className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 group flex flex-col h-full text-left w-full"
            >
              {/* Image Placeholder */}
        ...
        <div className="mt-8 md:hidden text-center">
           <button 
             onClick={() => onNavigate && onNavigate('공지사항')}
             className="inline-flex items-center gap-2 text-[#2F4F4F] font-semibold"
           >
              전체 게시글 보기
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
           </button>
